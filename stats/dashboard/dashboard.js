...
function showTip(html, event) {
  ...
  tipEl.style.left =
    (x + tipEl.offsetWidth > vw ? x - tipEl.offsetWidth - 28 : x) + "px";
...
function prepareData(shipments) {
  ...
  const byWeight = ["1", "2", "3", "4", "5", "6+"]
    .filter((k) => wtMap[k])
    .map((k) => ({
      label: k === "6+" ? "6+ lbs" : `${k} lb${k === "1" ? "" : "s"}`,
      count: wtMap[k],
      unknown: false,
    }));
  if (unknownCount > 0) {
    byWeight.push({ label: "Unknown", count: unknownCount, unknown: true });
  }
...
function renderHeaderStats(shipments) {
  ...
  const dests = new Set(
    shipments.map((r) => r.formatted_address || r.address).filter(Boolean),
  ).size;
  document.getElementById("stat-total-pkgs").textContent =
...
function renderMonthly(data) {
  ...
  const yP = d3
    .scaleLinear()
    .domain([0, d3.max(data, (d) => d.packages) * 1.15])
    .range([iH, 0]);
  const yC = d3
    .scaleLinear()
  ...
  g.selectAll(".bar")
    ...
    .attr("class", "bar")
    .attr("x", (d) => x(d.month))
    .attr("y", (d) => yP(d.packages))
    .attr("width", x.bandwidth())
    .attr("height", (d) => iH - yP(d.packages))
    .attr("fill", C.teal)
    .attr("rx", 2)
  ...
  g.append("g")
    .attr("class", "axis")
    .call(d3.axisLeft(yP).ticks(5).tickFormat(d3.format(",d")));
  g.append("g")
    .attr("class", "axis")
  ...
  const leg = svg
    .append("g")
    .attr("transform", `translate(${m.left + 8},${H - 10})`);
  leg
    .append("rect")
    .attr("width", 12)
    .attr("height", 8)
...
function renderDonut(data) {
  ...
  const g = svg
    .append("g")
    .attr("transform", `translate(${W * 0.42},${H / 2})`);
  const pie = d3
    .pie()
    .value((d) => d.count)
    .sort(null);
  ...
  g.selectAll(".arc")
    ...
    .append("path")
    .attr("d", arc)
    .attr("fill", (d) => CLASS_COLOR[d.data.name] || C.muted)
    .attr("stroke", "#0d1117")
    .attr("stroke-width", 2)
    .on("mousemove", (event, d) =>
      showTip(
  ...
  const leg = svg
    ...
    .attr(
      "transform",
      `translate(${W * 0.42 + radius + 18},${H / 2 - data.length * 8})`,
    );
  data.forEach((d, i) => {
    ...
    row
      ...
      .attr("height", 10)
      .attr("y", -1)
      .attr("fill", CLASS_COLOR[d.name] || C.muted)
      .attr("rx", 2);
    row
      .append("text")
      .attr("x", 15)
...
function renderWeight(data) {
  ...
  g.selectAll(".bar")
    ...
    .attr("fill", (d, i) =>
      d.unknown ? "rgba(160,160,160,0.6)" : barColors[i] || C.teal,
    )
    .attr("rx", 2)
    .on("mousemove", (event, d) =>
      showTip(
...
function renderLibraryWeight(data) {
  ...
  const xAvg = d3
    .scaleLinear()
    .domain([0, d3.max(data, (d) => d.avgLbs) * 1.3])
    .range([0, iW]);
  const y = d3
    .scaleBand()
...
window.addEventListener("load", async () => {
  try {
    ...
    const allShipments = data.shipments.filter((s) =>
