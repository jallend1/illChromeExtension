...
function prepareData(shipments) {
  ...
  const libMap = {};
  for (const s of shipments) {
    const name = s.recipient_company || s.recipient_name || '';
    if (!name) continue;
    libMap[name] = (libMap[name] || 0) + 1;
  }
  const byLibrary = Object.entries(libMap)
    .map(([name, count]) => ({ name, count }))
    .sort((a, b) => b.count - a.count)
    .slice(0, 15);
  ...
  for (const s of shipments) {
    const name = s.recipient_company || s.recipient_name || '';
    if (!name) continue;
    if (!libWtMap[name]) libWtMap[name] = { packages: 0, lbs: 0 };
    libWtMap[name].packages += 1;
    libWtMap[name].lbs      += parseFloat(s.weight_lbs || 0);
  }
  const byLibraryWeight = Object.entries(libWtMap)
    .map(([name, d]) => ({ name, lbs: d.lbs, packages: d.packages,
      avgLbs: d.packages > 0 ? d.lbs / d.packages : 0 }))
