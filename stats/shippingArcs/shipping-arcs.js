...
function buildStaticLayer() {
  return new deck.ArcLayer({
    ...
    getWidth: arcWidth, getHeight: arcHeight,
...
function buildAnimatedLayer() {
  return new deck.TripsLayer({
    id: 'trips', data: visibleShipments,
...
function initDeck() {
  deckInstance = new deck.DeckGL({
    ...
    mapLib: maplibregl,
    initialViewState: { longitude: -96, latitude: 40, zoom: 3.4, pitch: 40, bearing: -10 },
    controller: { dragRotate: true }, layers: [],
    onHover: showTooltip,
...
window.addEventListener('load', async () => {
  ...
  try {
    ...
    initSlider('arc-width-static', 'arc-width-static-val', v => v.toFixed(1), v => { arcWidth  = v; if (viewMode === 'static')   render(); });
    initSlider('arc-height',       'arc-height-val',       v => v.toFixed(1), v => { arcHeight = v; if (viewMode === 'static')   render(); });
    initSlider('anim-speed',       'speed-val',            v => v.toFixed(1) + '×', v => { animSpeed   = v; });
    initSlider('trail-length',     'trail-val',            v => String(Math.round(v)), v => { trailLength = v; if (viewMode === 'animated') render(); });
    initSlider('arc-width-anim',   'arc-width-anim-val',   v => v.toFixed(1), v => { arcWidthAnim = v; if (viewMode === 'animated') render(); });
