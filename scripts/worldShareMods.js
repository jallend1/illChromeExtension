...
(async () =>{
  ...
  const waitForElementWithInterval = (selectorOrFunction) =>
    ...
    // Select span element with data attribute of "requestStatus"
    const requestStatus = waitForElementWithInterval("[data='requestStatus']");
    if(!requestStatus) {
    ...
    if(requestStatus.innerText.includes("Recalled")) applyStyles(requestStatus);
        ...
        if(dispositionElement && dispositionElement.innerText.includes("Overdue")) {
